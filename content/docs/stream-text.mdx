...
<Step>
Open `src/stream-text.ts`. You should see the following code already:
```typescript title="src/stream-text.ts"
...
<Tabs items={['npm', 'pnpm']}>
  <Tab value="npm">
  ```bash
  npx tsx src/stream-text.ts
  ```
  </Tab>
  <Tab value="pnpm">
  ```bash
  pnpm tsx src/stream-text.ts
  ```
  </Tab>
</Tabs>
