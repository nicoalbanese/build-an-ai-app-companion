...
<Step>
Open `src/stream-object.ts`. You should see the following code already:

```typescript title="src/stream-object.ts"
...
<Tabs items={['npm', 'pnpm']}>
  <Tab value="npm">
  ```bash
  npx tsx src/stream-object.ts
  ```
  </Tab>
  <Tab value="pnpm">
  ```bash
  pnpm tsx src/stream-object.ts
  ```
  </Tab>
</Tabs>
