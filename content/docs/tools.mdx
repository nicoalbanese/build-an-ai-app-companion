...
<Step>
Open `src/tools.ts`. You should see the following code already:
    ...
    tools: {
      weather: {
        ...
        execute: async ({ location }) => {  // [!code highlight]
          const temperature = Math.floor(Math.random() * 31); // await getWeather(location) // [!code highlight]
    ...
    tools: {
      weather: {
        ...
        execute: async ({ location }) => {
          const temperature = Math.floor(Math.random() * 31); // await getWeather(location)
          return { temperature };
        },
      },
    },
